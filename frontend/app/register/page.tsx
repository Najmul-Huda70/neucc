...
import { signUp, syncFrontendSession } from "@/lib/auth-client";
  ...
  return (
    ...
    <div className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-xl flex-col justify-center px-4 py-6 sm:py-12">
      ...
      <div className="w-full rounded-[12px] border border-[var(--color-secondary)]/15 bg-white p-6 shadow-sm sm:p-8">
        <h1 className="text-xl font-bold text-[var(--color-primary)] sm:text-2xl">Join NEUCC</h1>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div>
            <label className="text-xs font-medium text-[var(--color-primary)] sm:text-sm">Full name</label>
            <input
              value={name}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="text-xs font-medium text-[var(--color-primary)] sm:text-sm">Student ID</label>
              <input
                value={studentId}
            ...
            <div>
              <label className="text-xs font-medium text-[var(--color-primary)] sm:text-sm">Batch / year</label>
              <select
                value={batch}
          ...
          <div>
            <label className="text-xs font-medium text-[var(--color-primary)] sm:text-sm">Email</label>
            <input
              type="email"
              value={email}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="text-xs font-medium text-[var(--color-primary)] sm:text-sm">Password</label>
              <input
                type="password"
                value={password}
            ...
            <div>
              <label className="text-xs font-medium text-[var(--color-primary)] sm:text-sm">Confirm password</label>
              <input
                type="password"
                value={confirm}
          ...
          <label className="flex items-start gap-2 text-xs text-[var(--color-neutral-text)] sm:text-sm">
            ...
            <span className="leading-tight">
              I agree to the{" "}
              <Link href="/terms" className="font-semibold text-[var(--color-secondary)] hover:underline">Terms</Link>{" "}
              and{" "}
              <Link href="/privacy" className="font-semibold text-[var(--color-secondary)] hover:underline">Privacy Policy</Link>.
            </span>
          </label>

          {error && <p className="text-xs text-[var(--color-danger)] sm:text-sm">{error}</p>}
        ...
        <p className="mt-6 text-center text-xs text-[var(--color-neutral-text)] sm:text-sm">
          Already a member?{" "}
          <Link href="/login" className="font-semibold text-[var(--color-secondary)] hover:underline">Log in</Link>
        </p>
      </div>
    </div>
  );
